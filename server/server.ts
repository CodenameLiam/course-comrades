...
app.post('/api/like-note', async (req, res) => {
  ...
  try {
    ...
    if (exists?.data()?.users.includes(username) !== undefined) {
...
app.post('/api/add-download', async (req, res) => {
  ...
  try {
    let docRef = await notesDB?.doc(noteId)?.get();

    let currentDownloads = docRef?.data()?.downloads?.get;
    // Updates downloads
    await currentDownloads.update({
...
app.post('/api/query/time/', async (req, res) => {
  ...
  switch (timePeriod) {
    case 'day':
      ...
      break;
    case 'week':
      fromDate = fromDate.subtract(7, 'days');
      break;
    case 'month':
      fromDate = fromDate.subtract(31, 'days');
      break;
    case 'year':
      fromDate = fromDate.subtract(365, 'days');
      break;
    default:
      fromDate = fromDate.subtract(7, 'days');
  }

  const timestampDate = admin.firestore.Timestamp.fromDate(fromDate.toDate());
  ...
  for (const note of sortedNotesArray) {
    ...
    if (refDoc?.data()?.notes.includes(note.id)) {
      note.liked = true;
    } else {
      note.liked = false;
    }
...
app.post('/api/search', async (req, res) => {
  try {
    ...
    // Hashtag query
    if (hashtags !== undefined) {
      const tags = (await hashtagQuery(hashtags)) as any;
    ...
    if (courseCode !== undefined) {
      notes = notes.filter((value) => value.get().courseCode === courseCode);
    }
